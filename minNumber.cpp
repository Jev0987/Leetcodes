...
//        int i = l, j = r;
//        while(i < j) {
//            while(strs[j] + strs[l] >= strs[l] + strs[j] && i < j) j--;//从右往左找第一个比基准小的值
//            while(strs[i] + strs[l] <= strs[l] + strs[i] && i < j) i++;//从左往右找第一个比基准大的值
//            swap(strs[i], strs[j]);//交换
//        }
//        swap(strs[i], strs[l]);//把基准与 从右往左找的第一个小的值（上面第一个while） 进行交换
